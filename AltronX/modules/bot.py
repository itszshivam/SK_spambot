...
async def ping(e):​🇸PY X DITTO
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» #​🇸𝘱𝘺 乂 DITTO", parse_mode=None, link_preview=None)
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"__🤖 🇸𝘱𝘺 乂 OP​__\n» `{mp} ms`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ok = await event.reply(f"» __​🇬addari_nahi_karna_​🇸py_x_se...__")
        mks = "SUDO_USER"
        target = ""
